function OxCM_profilometryData(nCpu, boundarClean, outlierClean, gx, gy, smth, flipSecondData, outputFilename)
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Parallel Iterative Closest Point (ICP) Algorithm
    %
    % The core registration, minimization, and extrapolation routines are 
    % adapted from the open-source implementation:
    %   "Iterative Closest Point" by Jakob Wilm & Hans Martin Kjer (2010)
    %   MATLAB Central File Exchange #27804
    %   Licensed under the BSD 2-Clause License.
    %   Copyright (c) 2010, Jakob Wilm. All rights reserved.
    %
    % Modifications and Parallelization by Fatih Uzun (2024):
    %   - Multi-core parallel matching via parfor (matchBruteForceParallel)
    %   - Parallelized normal estimation (estimateNormalsParallel)
    %   - Fallback parallel k-NN search routine (fallbackKnnParallel)
    %   - Levenberg-Marquardt optimization solver (solveLmaMin)
    %   - Integration into the automated OxCM profilometry data pipeline
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
